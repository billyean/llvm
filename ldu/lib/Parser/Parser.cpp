...
bool Parser::parseStatement(StmtList &Stmts) {
  auto _errorhandler = [this] {
    return skipUntil(
        tok::semicolon, tok::kw_ELSE, tok::kw_END);
  };
...
bool Parser::parseExpression(Expr *&E) {
  auto _errorhandler = [this] {
    return skipUntil(tok::r_paren, tok::comma, tok::semicolon,
                     tok::kw_DO, tok::kw_ELSE, tok::kw_END,
                     tok::kw_THEN);
  };
  if (parseSimpleExpression(E))
    return _errorhandler();
  if (Tok.isOneOf(tok::not_equal, tok::less, tok::less_equal,
                  tok::equal_to, tok::greater,
                  tok::greater_equal)) {
    OperatorInfo Op;
    Expr *Right = nullptr;
    if (parseRelation(Op))
...
bool Parser::parseRelation(OperatorInfo &Op) {
  ...
  if (Tok.is(tok::equal_to)) {
    ...
    Op = fromTok(Tok);
    advance();
  } else if (Tok.is(tok::less_equal)) {
    Op = fromTok(Tok);
    advance();
  } else if (Tok.is(tok::greater)) {
    Op = fromTok(Tok);
    advance();
  } else if (Tok.is(tok::greater_equal)) {
    Op = fromTok(Tok);
    advance();
  } else {
    /*ERROR*/
...
bool Parser::parseSimpleExpression(Expr *&E) {
  auto _errorhandler = [this] {
    return skipUntil(
        tok::not_equal, tok::r_paren, tok::comma, tok::semicolon,
        tok::less, tok::less_equal, tok::equal_to,
        tok::greater, tok::greater_equal, tok::kw_DO,
...
bool Parser::parseTerm(Expr *&E) {
  auto _errorhandler = [this] {
    return skipUntil(tok::not_equal, tok::r_paren, tok::plus,
                     tok::comma, tok::minus, tok::semicolon,
                     tok::less, tok::less_equal, tok::equal_to,
                     tok::greater, tok::greater_equal,
                     tok::kw_DO, tok::kw_ELSE, tok::kw_END,
...
bool Parser::parseFactor(Expr *&E) {
  auto _errorhandler = [this] {
    return skipUntil(
        tok::not_equal, tok::r_paren, tok::star,
        tok::plus, tok::comma, tok::minus,
        tok::slash, tok::semicolon, tok::less,
        tok::less_equal, tok::equal_to, tok::greater,
        tok::greater_equal, tok::kw_AND, tok::kw_DIV,
        tok::kw_DO, tok::kw_ELSE, tok::kw_END,
...
bool Parser::parseQualident(Decl *&D) {
  auto _errorhandler = [this] {
    return skipUntil(
        tok::not_equal, tok::l_paren, tok::r_paren,
        tok::star, tok::plus, tok::comma,
        tok::minus, tok::slash, tok::assign,
        tok::semicolon, tok::less, tok::less_equal,
        tok::equal_to, tok::greater, tok::greater_equal,
...
bool Parser::parseIdentList(IdentList &Ids) {
  auto _errorhandler = [this] {
    return skipUntil(tok::colon, tok::semicolon);
  };
  if (expect(tok::identifier))
    return _errorhandler();
